...
- **Email saliente:** centralizado en `lib/mailer.ts` (`getTransporter()` + `MAIL_FROM`). El buzón `hola@ialimp.es` está en **IONOS**, así que el transporte es **SMTP de IONOS** (`SMTP_USER` / `SMTP_PASSWORD`; host/puerto por defecto `smtp.ionos.es:465` SSL, override con `SMTP_HOST`/`SMTP_PORT`). Fallback a Gmail (`GMAIL_USER` / `GMAIL_APP_PASSWORD`) si no hay `SMTP_*`. El **remitente** (`from`) es **siempre `MAIL_FROM` = `hola@ialimp.es`** (display = nombre de la empresa). Si faltan credenciales, `getTransporter()` devuelve `null` y la ruta marca el correo como no enviado (no rompe). Rutas que mandan correo: `clientes/[id]/enviar-acceso`, `sesiones/[id]/completar`, `informes/generar`, `ia/agente-cotizador`.
