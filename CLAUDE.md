...
- **Agentes existentes (NO duplicar):**
  ...
  - **aiExtractProtocol** (`POST /api/admin/protocolos/extraer`): ingiere una ficha de limpieza (foto/escaneo → visión `llama-3.2-90b-vision`, o texto pegado → `aiComplete`) y devuelve un **borrador** de protocolo `{ nombre, estancias:[{estancia, items:[{descripcion, requiere_foto}]}], notas }` para **revisar antes de guardar** (NO escribe en BD). Guardar = `POST /api/admin/protocolos` (crea/reemplaza el protocolo activo del piso, desactiva el anterior —histórico—, genera `item_key` por estancia+orden; scope `empresa_id`). PDF/Word reales no se parsean (no hay `pdf-parse`/`mammoth` instalado) → entran como foto o texto. **Pendiente:** UI de subir→revisar→guardar.
