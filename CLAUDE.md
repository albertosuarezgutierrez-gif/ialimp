...
- **Recuperación de contraseña:** cubre las 3 cuentas con contraseña (`empresas`, `usuarios_empresa`, `superadmins`; col activa = `activa` / `activo` / `activo`). Flujo: `POST /api/auth/forgot-password {email}` (público, rate-limit por IP, **anti-enumeración**: responde siempre genérico) → busca el email por prioridad empresa→usuario→superadmin → guarda en `password_reset_tokens` el **hash SHA-256** del token (el claro va solo en el email; `genResetToken`/`hashToken` en `lib/auth.ts`), caduca en **1h**, un solo uso → email vía `getTransporter()` con enlace a `/recuperar/[token]`. `GET/POST /api/auth/reset-password` valida y fija la nueva contraseña (`hashPassword`, ≥8 chars), marca `used_at` e invalida el resto de tokens de la cuenta. Páginas públicas `/recuperar` y `/recuperar/[token]` (exentas en middleware). Enlaces "¿Olvidaste tu contraseña?" en `/login` y `/superadmin`. Tras resetear NO auto-login (redirige a login). (Limpiadoras=PIN y propietarios=enlace NO tienen contraseña → fuera de este flujo.)
