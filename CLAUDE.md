...
- **`catalogo_tarifas`** (empresa-scoped, SQL crudo): lista de conceptos (pisos/servicios) con `tiempo_min`, `precio` (= pago a la limpiadora, base de la nómina) y `precio_cliente` (opcional). `categoria` ∈ `piso` | `servicio` | `pc`. Único `(empresa_id, lower(nombre))`. **`pc`** = filas «PC …» del Excel de Sique Brilla importadas literalmente, **pendientes de confirmar su significado** (¿pago a la limpiadora del piso?, ¿otro servicio?) → se dejan aparte, NO mezclar con su piso «padre» hasta aclararlo con Vanessa. Migración: `prisma/migrations/add_catalogo_tarifas_partes_trabajo.sql` (ya aplicada + seed de 196 conceptos para Sique Brilla; el `PC GALINDO` duplicado 5,0€ se omitió, queda el 5,5€).
