...
- **Agentes existentes (NO duplicar):**
  ...
  - **aiExtractProtocol** (`POST /api/admin/protocolos/extraer`): ingiere una ficha de limpieza (foto/escaneo → visión `llama-3.2-90b-vision`, o texto pegado → `aiComplete`) y devuelve un **borrador** de protocolo `{ nombre, estancias:[{estancia, items:[{descripcion, requiere_foto}]}], notas }` para **revisar antes de guardar** (NO escribe en BD). Guardar = `POST /api/admin/protocolos` (crea/reemplaza el protocolo activo del piso, desactiva el anterior —histórico—, genera `item_key` por estancia+orden; scope `empresa_id`). PDF/Word reales no se parsean (no hay `pdf-parse`/`mammoth` instalado) → entran como foto o texto. **UI:** `components/ProtocoloModal.tsx`, botón "📋 Protocolo" por piso en Negocio › Clientes › Propiedades (subir ficha/pegar texto → lista editable de tareas → guardar).
...
- **Minutos reales:** `cleaning_sessions.tiempo_estimado` suele venir NULL → carga y tope se calculan con `COALESCE(cs.tiempo_estimado, propiedades.duracion_estimada_min, 120)` (la duración vive en la ficha del piso, poblada en todas). Mismo cálculo en `auto-assign` y en la lista de override `asignacion`.
- **Tipo de limpieza = `cleaning_sessions.tipo_servicio`** (canónico; `tipo_limpieza` es residual, solo 3 sitios). Sale del catálogo **editable** `tipos_servicio_op` (Configuración › Catálogos). Cada tipo tiene un **`factor` de tiempo editable** (rotación 1, profunda 1.5, obra 2…): el scoring multiplica los minutos por ese factor vía un CTE `factores` que desanida el jsonb del catálogo (`jsonb_array_elements`, guardado con `jsonb_typeof = 'array'`). Retrocompatible: sin `factor` ⇒ 1 (catálogos ya sembrados no cambian hasta que la dueña edite el factor). Aplicado en `auto-assign` (est de la sesión + carga) y en `asignacion` (carga mostrada).
- **Scoring v3 (vigente) — lógica de Vanessa:** `auto-assign` hace 2 pases. **Pase 1 hotel:** agrupa sesiones `propiedades.tipo='habitacion_hotel'` por día; ≤4→1 persona, >4→2; reparte minutos greedy (hab más larga → menos cargada). **"Cuenta y sigue"** (decisión Alberto): las elegidas conservan esa carga y SIGUEN como candidatas en el pase 2 (su carga sube → el scoring las deprioriza solo). **Pase 2 apartamentos:** ordena por salida más temprana (`hora_checkout`) y procesa primero las que tienen entrada el mismo día (`hora_checkin_siguiente IS NOT NULL`); como cada asignación sube la carga, las entradas se reparten entre limpiadoras distintas.
...
- **Pendiente:** el indicador de entrada (`hora_checkin_siguiente`) solo está poblado en back-to-back de Smoobu; para pisos externos por iCal no siempre llega → "repartir entradas" es best-effort hasta mejorar el alta. La rotación por descansos del pool del hotel hoy se decide por carga (no hay rotación explícita por descanso semanal).
