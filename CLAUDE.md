...
- Auto-asignación real: `GET /api/admin/auto-assign`, solo toca sesiones de hoy+mañana con `limpiadora_id` NULL (lo ya asignado no se mueve). Crons 5:30 y 16:00 (hora España, DST-proof). Scoring actual prioriza `conoce_propiedad` y desempata por carga.
...
- **Mejora pendiente (scoring v2):** `conoce_propiedad` es absoluto y la carga solo desempata → reparto desigual; el bucket de carga se satura a 4 h. Propuesta: carga continua + tope por `horas_max`; clave de piso unificada `COALESCE(NULLIF(propiedad_id,''), property_id)`. **Lógica de Vanessa a modelar:** hotel ≤4 limpiezas→1 persona, >4→2; equilibrar minutos por persona; repartir entradas; empezar por salidas más tempranas. **Decisión pendiente de Alberto/Vanessa:** si la del hotel por la mañana sigue con apartamentos (su balance de minutos cuenta el hotel) o es jornada completa.
