...
export default async function PropietarioPage({
  ...
  const propiedades = await prisma.$queryRaw<any[]>(Prisma.sql`
    SELECT
      ...
      cs_hoy.foto_url,
      cs_hoy.hora_checkout,
      cs_hoy.hora_checkin_siguiente,
      ultima.ultima_limpieza,
      ultima.ultima_foto
    FROM propiedades p
    LEFT JOIN LATERAL (
      SELECT
      ...
      WHERE cs.propiedad_id = p.id
        AND cs.session_date = CURRENT_DATE
      LIMIT 1
    ) cs_hoy ON true
    LEFT JOIN LATERAL (
      SELECT
        MAX(cs.session_date) AS ultima_limpieza,
        (SELECT foto_despues_url FROM cleaning_sessions
         WHERE propiedad_id = p.id AND foto_despues_url IS NOT NULL
         ORDER BY session_date DESC LIMIT 1) AS ultima_foto
      FROM cleaning_sessions cs
      WHERE cs.propiedad_id = p.id AND cs.completed_at IS NOT NULL
    ) ultima ON true
    WHERE p.cliente_id = ${cliente.id}::uuid AND p.activa = true
    ORDER BY p.nombre ASC
  `)

  // Últimas limpiezas (historial)
