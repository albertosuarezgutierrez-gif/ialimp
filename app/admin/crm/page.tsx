...
const ESTADOS = {
  ...
  presupuestado:  { label: 'Presupuestado',  color: '#7c3aed',bg: '#faf5ff'},
...
export default function CRMPage() {
  ...
  async function guardarNota(id: string) {
    if (!nota.trim()) return
    await fetch('/api/admin/leads', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, notas: nota }) })
  ...
  const pendientes  = leads.filter(l => l.estado === 'nuevo' || l.estado === 'contactado').length
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: "'DM Sans', sans-serif" }}>
      <header style={{ background: C.primary, padding: '18px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 12 }}>{total_leads} leads · {pendientes} pendientes · {ganados} ganados</p>
        </div>
        <a href="/cotizador" target="_blank" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '8px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
          🔗 Cotizador público
        </a>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selLead ? '1fr 380px' : '1fr', gap: 0, maxHeight: 'calc(100vh - 120px)', overflow: 'hidden' }}>
        {/* Lista */}
        <div style={{ padding: '16px 24px', overflowY: 'auto' }}>
          ...
          {!loading && filtrados.length === 0 && (
            <div style={{ textAlign: 'center', padding: 40, color: C.muted }}>
              ...
              <p style={{ fontSize: 13, marginTop: 4 }}>Los leads del cotizador aparecerán aquí</p>
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {filtrados.map((lead: any) => {
              const est = ESTADOS[lead.estado as keyof typeof ESTADOS] || ESTADOS.nuevo
              return (
                <div key={lead.id} onClick={() => setSelLead(lead)}
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: C.text, marginBottom: 3 }}>{lead.nombre}</div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      {lead.precio_estimado && <div style={{ fontWeight: 800, color: C.primary, fontSize: 15 }}>{lead.precio_estimado}€</div>}
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                {[
                  ...
                  ['💶', selLead.precio_estimado ? selLead.precio_estimado + '€ estimado' : null, null],
