...
async function getAuth() {
  ...
  const rows = await prisma.$queryRaw<any[]>(Prisma.sql`
    ...
    WHERE s.token = ${token}::uuid LIMIT 1
  `)
  return rows[0] || null
}
...
export async function GET(req: Request) {
  try {
    ...
    const mensajes = await prisma.$queryRaw<any[]>(Prisma.sql`
      ...
      WHERE empresa_id = ${auth.empresa_id}::uuid
    ...
    await prisma.$executeRaw(Prisma.sql`
      ...
      WHERE empresa_id = ${auth.empresa_id}::uuid AND remitente_tipo = 'admin'
...
export async function POST(req: Request) {
  try {
    ...
    await prisma.$executeRaw(Prisma.sql`
      ...
      VALUES (${auth.empresa_id}::uuid, ${sesion_id || null}, 'limpiadora',
              ${auth.limpiadora_id}::uuid, ${auth.nombre}, ${texto})
    `)
