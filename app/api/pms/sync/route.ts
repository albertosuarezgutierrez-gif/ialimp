...
// ── Mapeo Smoobu apartment ID → propiedades ialimp ───────────────────────────
    ...
    try {
      await prisma.$executeRaw(Prisma.sql`
        INSERT INTO cleaning_sessions (
          ...
          ${conn.empresa_id}::uuid, ${conn.cliente_id || null},
          ${conn.id}::uuid, ${conn.id}, ${conn.cliente_nombre},
          ${icalDateToISO(ev.dtend)}::date,
          ${conn.id + '_' + ev.uid}, ${ev.summary || null}, 'rotacion'
        )
        ON CONFLICT (external_reservation_id)
