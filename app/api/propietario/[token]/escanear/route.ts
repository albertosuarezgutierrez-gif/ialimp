...
// POST: recibe imagen base64, la analiza con Claude Vision,
//       guarda en documentos_contables y actualiza stock si procede
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: 'text',
              text: `Eres contable experto español. Lee esta imagen y devuelve SOLO JSON válido sin texto extra ni backticks:
{
...
es_stock=true solo para artículos físicos. null si ilegible.`,
    ...
    try {
      ...
      return NextResponse.json({ error: 'IA no pudo leer el documento' }, { status: 422 })
    }

    // ── 2. Generar apunte PGC ─────────────────────────────────
    ...
    for (const linea of lineasStock) {
      ...
      if (existing.length > 0) {
        ...
        `)
        // Registrar movimiento en stock_consumos (cantidad positiva = entrada)
        await prisma.$executeRaw(Prisma.sql`
        ...
        await prisma.$executeRaw(Prisma.sql`
          ...
          VALUES (
            ${cliente.empresa_id}::uuid,
            ${linea.descripcion},
            ${linea.categoria_stock || 'limpieza'},
            ${linea.unidad || 'unidad'},
            ${Number(linea.cantidad || 1)},
            0,
    ...
    // Marcar doc como procesado si hubo stock
